...
def tokenize(expression):
    ...
    expression = re.sub(
        r"(\d+|\))\s*\(", r"\1*(", expression
    )  # Handle cases like "2(" as "2*("
    ...
    while i < length:
        ...
        if (
            char.isdigit()
            or char == "."
            or (char == "-" and (i == 0 or expression[i - 1] in "(+-*/^√"))
        ):
            number = char
            i += 1
...
class TestTrigAndRootFunctions(unittest.TestCase):
    ...
    def test_nth_root(self):
        ...
        self.assertIsNone(
            nth_root(-16, 2)
        )  # Test for even root of negative number (should return None)

    def test_nth_root_complex(self):
        result = nth_root(-8, 3)
        self.assertAlmostEqual(
            result, -2, places=5
        )  # Test for negative number with odd root
